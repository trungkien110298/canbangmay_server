...
function display_result(data) {
	...
	for (let i in groups) {
		...
		for (let task in group.tasks) {
			...
			let td_task = $("<td></td>").append(
				$("<p></p>", { class: "text-center" }).html(nccn.task)
			);
			td_task.appendTo(rows[task]);

			let td_NCCN = $("<td></td>").append(
				$("<p></p>", { class: "text-center" }).html(
					list_NCCN[parseInt(nccn.task) - 1].name
				)
			);
			td_NCCN.appendTo(rows[task]);

			let td_machine = $("<td></td>").append(
				$("<p></p>", { class: "text-center" }).html(nccn.machine)
			);
			td_machine.appendTo(rows[task]);

			let td_ti = $("<td></td>").append(
				$("<p></p>", { class: "text-center" }).html(nccn.ti)
			);
	...
	//Display
	//display_table(groups, num_groups, list_NCCN);
...
function display_graph(group_data) {
	...
	const width = document.getElementById("assembly_line-graph").scrollWidth;
