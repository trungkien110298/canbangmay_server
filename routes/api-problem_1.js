...
api_problem_1.post("/api-problem_1", (req, res) => {
	...
	if (problem.cycle_time != NaN) {
		...
		cycle_time = parseInt(time) * parseInt(wattage);
	}
	...
	cmd.get(
		...
		function(err, data, stderr) {
			...
			output_data.rmin = parseInt(cycle_time) - (parseInt(cycle_time) * deviation) / 100;
			output_data.rmax = parseInt(cycle_time) + (parseInt(cycle_time) * deviation) / 100;
